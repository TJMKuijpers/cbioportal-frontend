...
> {
    ...
    @computed get showSamplesPerStudy() {
        const shownStudies = this.logic.mainView.getSelectionReport()
            .shownStudies;
        const studyForCalculation = shownStudies.length < this.store.cancerStudies.result.length?shownStudies:this.store.cancerStudies.result
        const filterAttributes = StudyFilterOptionsFormatted.filter(item=> item.name)
        const sampleCountsPerFilter = StudyFilterOptionsFormatted.map(item => item.id).map(filter => {
                                ...
                                <div>
                                    ...
                                    >
                                        <DataTypeFilter
                                            ...
                                            store={this.store}
                                            samplePerStudy={this.showSamplesPerStudy}
                                        />
                                    </div>
                                    <div
